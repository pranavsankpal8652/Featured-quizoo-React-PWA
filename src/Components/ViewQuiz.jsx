...
import { getDatabase, ref, onValue, remove, set, update, limitToFirst, query, get, orderByKey, endBefore, limitToLast } from "firebase/database";
...
export default function ViewQuiz() {
    ...
    useEffect(() => {
        async function loadQuizess() {
            ...
            if (snapshot.val()) {
                ...
                setQuizess(prev=>({...quizessList,...prev})); //to state

                // // Enable Load More button only if the total blogs fetched exceed 6
                setLoadMore(Object.keys(quizessList).length >= 1);
            } else {
    ...
    const loadmore = async () => {
        ...
        const quizessQuery = query(quizessRef, orderByKey(), endBefore(lastKey), limitToLast(5));
        ...
        if (snapshot.val()) {
            ...
            const totalQuizfetched = Object.keys(quizess).length + Object.keys(quiz).length;
                ...
                <div className={`text-center flex justify-center items-center h-screen ${loading ? 'block' : 'hidden'}`}>
                    <div role="status">
                        <svg aria-hidden="true" className="inline w-12 h-12 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="blue" />
                        ...
                        <>
                            {
                                Object.entries(quizess).map(([key, quiz], index) => {
                                    ...
                                    return (
                                        <div className="relative max-w-[1340px] mx-auto px-6 py-6 shadow-gray-700 shadow-xl mb-[50px]" key={key}>
                                            <div className='text-black font-bold italic underline p-2 text-lg'>{key}</div>
                                                        ...
                                                        quiz.map((question, index) => {
                                                            return (
                                                                <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 " key={index}>
                                                                    <th scope="col" className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white max-w-[500px] text-wrap">
                                                                        {question.title}
                                                                    </th>
                                            ...
                                            </table>
                                            {
                                                ...
                                                <>
                                                    <div className='w-fit mx-auto my-7'>
                                                        <button className='text-sm text-white bg-green-600 hover:bg-green-800 rounded-lg p-[10px_20px]' onClick={() => publishQuiz(quiz, key)}>
                                                            {Publish[key] ? 'Unpublish Quiz' : 'Publish Quiz'}
                                                        </button>
                                                    </div>
                                                    <div className='w-fit mx-auto mt-7'>
                                                        <button className='text-sm text-white bg-cyan-800 hover:bg-slate-500 rounded-lg p-[10px_20px]' onClick={() => deleteQuiz(quiz, key)}>
                                                            Delete This Quiz
                                                        </button>
                                                    </div>
                                                </>
                            ...
                            <div className={`mx-auto w-fit py-10 ${loadMoreQuizess ? 'block' : 'hidden'}`}>
                                <button className='text-sm text-white bg-gradient-to-r from-violet-700 via-blue-500 to-blue-300 brightness-95 p-[10px_20px] rounded-lg ' onClick={loadmore}>
                                    Load More
                                </button>
                            </div>
                        </>
                        :
                        <div className={`text-4xl text-red-600 text-center flex justify-center items-center h-screen overflow-hidden1 ${loading ? 'hidden' : 'block'}`}>
                            No Quizess Yet
                        </div>
                }
